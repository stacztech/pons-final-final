...
@Component({
  ...
  template: `
    <div class="container my-5">
      <h1 class="section-title">Goat</h1>
...
export class GoatComponent implements OnInit {
  ...
  hasSelectedWeight(product: GoatProduct): boolean {
    return (product.quantity1kg || 0) > 0 || (product.quantity500g || 0) > 0;
  }

  isProductInCart(product: GoatProduct): boolean {
  ...
  increaseQuantity(product: GoatProduct, weightType: '1kg' | '500g') {
    if (weightType === '1kg') {
  ...
  decreaseQuantity(product: GoatProduct, weightType: '1kg' | '500g') {
